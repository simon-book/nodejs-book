...
//抓取m.35xs.co的小说
var branch = branchMap["m.35wx.com"];
...
async function create_book(bookHref) {
    try {
        ...
        var bookHtml = await httpGateway.gbkHtmlStartReq(branch.copyUrl, book.copyInfo.a);
        var $ = cheerio.load(bookHtml, {
            decodeEntities: false
        });
        var liItems = $(".synopsisArea_detail").children();
        book.cover = $(liItems[0]).attr("src");
        book.title = $(liItems[1]).text();
        console.log(book.title);
        book.writer = $(liItems[2]).text().slice(3);
        book.categoryName = $(liItems[3]).text().slice(3) || "其他小说";
        book.categoryId = branch.category[book.categoryName][1];
        book.publishStatus = branch.publishStatus[$(liItems[4]).text().slice(3)];
        book.lastUpdatedAt = new Date($(liItems[6]).text().split("：")[1]);
        book.abstractContent = $("p.review").text().replace(/\s/g, "").slice(5);
        var sameBook = await bookSequelize.findOneBook({
            title: book.title,
            writer: book.writer
        })
        if (sameBook) return true;
        var muluHtml = await httpGateway.gbkHtmlStartReq(book.copyInfo.m);
        var $ = cheerio.load(muluHtml, {
            decodeEntities: false
        });
        var chapters = $("#list dl").children();
        var dd0Index = $(chapters).indexOf($("#list dt")[1]);
...
async function update_book(savedBook) {
    try {
        var bookHtml = await httpGateway.gbkHtmlStartReq(branch.copyUrl, savedBook.copyInfo.a);
        var $ = cheerio.load(bookHtml, {
            decodeEntities: false
        });
        var liItems = $(".synopsisArea_detail").children();
        var lastUpdatedAt = new Date($(liItems[6]).text().split("：")[1]);
        if (Math.abs(lastUpdatedAt.getTime() - new Date(savedBook.lastUpdatedAt).getTime()) > 10000) {
            ...
            savedBook.set("publishStatus", branch.publishStatus[$(liItems[4]).text().slice(3)]);

            var newChapters = [];
            var muluHtml = await httpGateway.gbkHtmlStartReq(savedBook.copyInfo.m);
            var $ = cheerio.load(muluHtml, {
                decodeEntities: false
            });
            var chapters = $("#list dl").children();
